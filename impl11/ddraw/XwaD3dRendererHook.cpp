	...
	float *diffuseMipLevel_out)
{
	...
	*diffuseMipLevel_out = 0;
	...
	{
		// cubeFace is declared outside this loop: release whatever the previous iteration
		// loaded, so a stale pointer can't be copied again if this face fails to load.
		if (cubeFace != nullptr)
		{
			cubeFace->Release();
			cubeFace = nullptr;
		...
		if (!g_config.EnableSideProcess || !g_bXwaDDrawPlayerPresent)
		{
			...
			// NOTE: do NOT redeclare res here. Shadowing it hides load failures from the
			// SUCCEEDED(res) test below, which then dereferences a null cubeFace.
			res = DirectX::CreateWICTextureFromFile(device, context,
				fileNames[cubeFaceIdx].c_str(), (ID3D11Resource**)&cubeFace, &cubeFaceSRV);
			if (cubeFaceSRV != nullptr) cubeFaceSRV->Release();
		}
		else
		{
			HANDLE handle;
			res = ExeLoadTexture(fileNames[cubeFaceIdx], &handle);
		...
		// The null check is not redundant: a loader can return a success code without
		// producing a texture. Dereferencing cubeFace below would then crash.
		if (SUCCEEDED(res) && cubeFace != nullptr)
		{
		...
		{
			// Diagnostic: this path is reachable on Linux/wine, where the WIC loader can
			// fail for cube faces that load fine on Windows. Name the file and the reason.
			char faceName[MAX_PATH] = { 0 };
			WideCharToMultiByte(CP_ACP, 0, fileNames[cubeFaceIdx].c_str(), -1,
				faceName, MAX_PATH, nullptr, nullptr);
			log_debug("[DBG] [CUBE] COULD NOT LOAD CUBEFACE [%d] (%s). Error: 0x%x, cubeFace: %s",
				cubeFaceIdx, faceName, res, cubeFace == nullptr ? "null" : "non-null");
			if (cubeFace != nullptr) { cubeFace->Release(); cubeFace = nullptr; }
			return false;
		}
	}
