...
public:
	XwaTextureData();
	void Clone(XwaTextureData* dst);
	void TagTexture();
	bool LoadShadowOBJ(const char* sFileName);

	int LoadDATImage(const char* sDATFileName, int GroupId, int ImageId, bool cache,
		ID3D11ShaderResourceView** srv, short* Width_out = nullptr, short* Height_out = nullptr);

	int LoadZIPImage(const char* sZIPFileName, int GroupId, int ImageId, bool cache,
		ID3D11ShaderResourceView** srv, short* Width_out = nullptr, short* Height_out = nullptr);

	std::string GetDATImageHash(const char* sDATZIPFileName, int GroupId, int ImageId);
	int GetCachedSRV(const char* sDATZIPFileName, int GroupId, int ImageId, ID3D11ShaderResourceView** srv);
	int AddCachedSRV(const char* sDATZIPFileName, int GroupId, int ImageId, int index);
