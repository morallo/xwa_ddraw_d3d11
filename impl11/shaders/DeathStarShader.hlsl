...
//IMPL_SATURATE( float )
//IMPL_SATURATE( float2 )
//IMPL_SATURATE( float3 )
//IMPL_SATURATE( float4 )
...
float calc_angle( vec2 v ) { return atan2( v.y, v.x ); }
...
vec3 luminance( vec3 c ) {
	float d = dot(vec3(0.2989, 0.5866, 0.1145), c);
	return vec3(d, d, d); 
}
...
//REPEAT_FUNCTIONS( float2, bounds2 )
float2 repeat(float2 x, float2 len) {
	float2 temp;
	temp = len * fract(x * (float2(1.0, 1.0) / len));
	return temp;
}

float2 repeat_mirror(float2 x, float2 len) { 
	float2 temp = len * abs(float2(-1.0, -1.0) + 2.0 * fract(((x * (float2(1.0, 1.0) / len)) - float2(-1.0, -1.0)) * 0.5)); 
	return temp;
}

//REPEAT_FUNCTIONS( float3, bounds3 )
float3 repeat(float3 x, float3 len) {
	return len * fract(x * (float3(1.0, 1.0, 1.0) / len));
}

float3 repeat_mirror(float3 x, float3 len) {
	return len * abs(float3(-1.0, -1.0, -1.0) + 2.0 * fract(((x * (float3(1.0, 1.0, 1.0) / len)) - float3(-1.0, -1.0, -1.0)) * 0.5));
}
...
#define TRI_FUNCTIONS2(type, btype) \
/* y=1-x on 0,1, output 0,1 */ \
type tri0( type x ) { return abs( fract( x * 0.5 ) - type(0.5, 0.5) ) * 2.0; } \
/* function returns 0 at x = half_width */ \
type tri_p( type x, type half_width, type half_period )	{ return half_width - repeat_mirror( x, half_period ); }

#define TRI_FUNCTIONS3(type, btype) \
/* y=1-x on 0,1, output 0,1 */ \
type tri0( type x ) { return abs( fract( x * 0.5 ) - type(0.5, 0.5, 0.5) ) * 2.0; } \
/* function returns 0 at x = half_width */ \
type tri_p( type x, type half_width, type half_period )	{ return half_width - repeat_mirror( x, half_period ); }

TRI_FUNCTIONS( float, bounds1 )
TRI_FUNCTIONS2( vec2, bounds2 )
TRI_FUNCTIONS3( vec3, bounds3 )
...
{
	vec4 d = vec4(FLT_MAX, FLT_MAX, FLT_MAX, FLT_MAX);
...
mat4 look_around_mouse_control( mat4 camera, float pitch, float tan_half_fovy )
{
	...
	mm.x = -mm.x;
	mm = sign( mm ) * pow( abs( mm ), vec2(0.9, 0.9) );
...
float warpTrace( Ray ray )
{
	...
	vec3 dd = vec3(FLT_MAX, FLT_MAX, FLT_MAX);
...
float star_glare( float x, float e, float c ) { return exp2( -pow(abs(x), e ) * c ); }
...
{
	float m = .01 + 5. * m1;
	vec3 f = vec3(1.0, 1.0, 1.0) + 0.5 * vec3( 0., 1., 2. );
	float Trcp = 1. / T;
...
#if 1
	c *= 1e+6;
	float max_comp_col = maxcomp(col);
	col = mix( col, vec3(max_comp_col, max_comp_col, max_comp_col), 0.5 ); // after all those efforts we lerp towards monotone... :-]
...
vec3 gamma_correction( vec3 L ) { return pow(L, 0.45); }
// mentioned in http://resources.mpi-inf.mpg.de/tmo/logmap/
vec3 gamma_correction_itu( vec3 L ) { return mix( 4.5061986 * L, 1.099 * pow(abs(L), 0.45) - 0.099, step(0.018, L) ); }
...
//	col = max( vec3( 0. ), contrast( col, vec3( 1.015 ) ) );
	...
	float vignette = .0 + 1. * pow(abs(20. * uv.x * uv.y * ( 1. - uv.x ) * ( 1. - uv.y )), 0.075 ); 
	col *= vignette;
...
vec3 laser_heatmap(float u) { return 0; }
...
//float4 main(in vec2 fragCoord ) : SV_TARGET
...
//	fragColor.xyz = laser_heatmap( uv.x ); return;
...
#else
	{
...
#ifdef CAMERA_REPEAT
		float camera_select = floor( time / time_slice ) % 2.0;
		...
		camera = get_camera2(camera, tan_half_fovy, time + CAM2_TIME_OFFSET, debug_color, exposure);
		//CameraRet cam = init_cam();
		//camera = lookat(float3(0, 0, 0), float3(0, 0, 1), float3(0, 1, 0));
	}
#endif

	Ray view_ray = get_view_ray( (uv - 0.5) * 2.0, znear, aspect, tan_half_fovy );

	view_ray.o = camera[3].xyz;
	//view_ray.o = 0;
	//view_ray.d = ( camera * vec4( view_ray.d, 0.0 ) ).xyz;
	view_ray.d = mul(camera, vec4(view_ray.d, 0.0)).xyz;
